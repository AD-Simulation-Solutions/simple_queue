...
def generate_message(message_size: int) -> Dict[str, Any]:
  ...
  base_msg = {"id": 0, "timestamp": time.time(), "data": ""}
...
def consumer_thread(
  ...
  try:
    ...
    with queue_consumer(queue_name=queue_name, uri=rabbit_uri, queue_size=queue_size) as consumer:
...
def main():
  ...
  parser.add_argument("--queue-size", "-q", type=int, default=100, help="Maximum queue size")
