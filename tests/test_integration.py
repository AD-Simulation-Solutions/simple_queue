...
class TestIntegration:
    ...
    def test_publisher_consumer_integration(
        self, mock_message_class, mock_exchange_class, mock_queue_class, mock_conn_class
    ):
        ...
        # Test publisher
        with QueuePublisher(
            "test_queue", "amqp://localhost", 1000, 3, 1.0
        ) as publisher:
...
class TestEdgeCases:
    ...
    def test_publisher_push_with_zero_retries(
...
class TestErrorHandling:
    ...
    def test_publisher_context_manager_exception_in_setup(self):
        ...
        with patch("simple_queue.core.QueueConnection") as mock_conn_class:
            ...
            with patch.object(
                publisher, "_setup_queue", side_effect=Exception("Setup failed")
            ):
    ...
    def test_consumer_context_manager_exception_in_setup(self):
        ...
        with patch("simple_queue.core.QueueConnection") as mock_conn_class:
            ...
            with patch.object(
                consumer, "_setup_queue", side_effect=Exception("Setup failed")
            ):
                with pytest.raises(Exception, match="Setup failed"):
                    with consumer:
                        pass
