...
class TestPublisherExample:
    ...
    def test_publisher_main_handles_failed_messages(
        self, mock_print, mock_queue_publisher, mock_sleep
    ):
    ...
    @patch("examples.publisher_example.queue_publisher")
    @patch("builtins.print")
    def test_publisher_main_prints_status_messages(
        self, mock_print, mock_queue_publisher, mock_sleep
    ):
        ...
        init_calls = [
            call
            for call in mock_print.call_args_list
            if "✅ Publisher initialized" in str(call)
        ]
        ...
        finish_calls = [
            call
            for call in mock_print.call_args_list
            if "✅ Finished sending messages" in str(call)
        ]
        assert len(finish_calls) == 1

        # Check for sending messages (should be 100)
        send_calls = [
            call for call in mock_print.call_args_list if "📤 Sending:" in str(call)
        ]
...
class TestConsumerExample:
    ...
    def test_consumer_main_processes_messages(self, mock_print, mock_queue_consumer):
        ...
        received_calls = [
            call for call in mock_print.call_args_list if "📥 Received:" in str(call)
        ]
        ...
        shutdown_calls = [
            call
            for call in mock_print.call_args_list
            if "✅ Consumer shutdown complete" in str(call)
        ]
    ...
    def test_consumer_main_handles_exception(self, mock_print, mock_queue_consumer):
        ...
        error_calls = [
            call
            for call in mock_print.call_args_list
            if "❌ Consumer error:" in str(call)
        ]
        assert len(error_calls) == 1

    @patch("examples.consumer_example.queue_consumer")
    @patch("builtins.print")
    def test_consumer_main_prints_startup_messages(
        self, mock_print, mock_queue_consumer
    ):
        ...
        startup_calls = [
            call
            for call in mock_print.call_args_list
            if "🔄 Starting continuous consumer" in str(call)
        ]
        ...
        ready_calls = [
            call
            for call in mock_print.call_args_list
            if "✅ Queue 'task_queue' ready" in str(call)
        ]
