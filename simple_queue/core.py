...
class QueueNotFoundError(Exception):
  """Exception raised when attempting to publish to a non-existent queue.

    This exception is raised by QueuePublisher.push() when the target queue
    does not exist in RabbitMQ. It provides a clear indication that the queue
    needs to be created before publishing messages.

    Attributes:
        queue_name (str): The name of the queue that was not found
...
class QueuePublisher:
  ...
  def __init__(
    ...
    log.debug(
        f"Initialized QueuePublisher for queue '{queue_name}' with retry times: {retry_times}, retry delay: {retry_delay}, queue size: {queue_size}")
  ...
  def _ensure_connected_and_queue_exists(self) -> None:
    """Ensure connection is active and queue exists, raise exception if queue doesn't exist.

        This method extends the basic connection check to also verify that the target
        queue exists in RabbitMQ. If the queue doesn't exist, it raises a QueueNotFoundError
        to prevent publishing to a non-existent queue.

        Raises:
            QueueNotFoundError: If the queue doesn't exist in RabbitMQ
    ...
    try:
      ...
      queue.declare(passive=True)  # passive=True means only check existence, don't create
    except Exception as e:
      raise QueueNotFoundError(
          self.queue_name,
          "Queue seams to be deleted. Restart publisher to create it",
      ) from e
  ...
  def push(self, message_data: Union[dict, list, str]) -> bool:
    """Push message to queue with retry logic and automatic serialization.
        ...
        Behavior:
            - Checks queue existence before attempting to publish
    ...
    for attempt in range(self.retry_times):
      try:
        # Ensure connection and verify queue exists
        self._ensure_connected_and_queue_exists()
        ...
        if published:
          log.info(f"Published to queue '{self.queue_name}': "
                   f"{message_body[:100]}...")
          return True
        else:
      ...
      try:
        ...
        for message in self._queue.consume_messages():
          try:
            ...
            log.debug(f"Received message from '{self.queue_name}': "
                      f"{body[:100]}...")
